...
<br>
　・ポート設計 8082(Nginx)や8081(phpMyAdmin)など、ローカル環境の他プロジェクトと衝突しないよう配慮した設計。
<br>
...
<br>
1. DB設計:<br><br>
・休憩時間を別テーブルで管理することで、1日に複数回の休憩をとるケースにも対応できる拡張性を持たせました。<br><br>
2. UI/UX:<br><br>
・打刻状態（現在「出勤中」なのか「休憩中」なのか）を動的に判断し、次に押すべきボタンのみを活性化させることで、誤操作を防ぐ設計にしました。<br><br>
3. 環境構築の自動化:<br><br>
・docker-compose up だけで開発環境が立ち上がるよう、設定を最適化しました。<br><br>
4. バリデーションと認可:<br><br>
・Requestクラスを用いた入力値バリデーションの徹底。<br><br>
・管理者(Admin)と一般ユーザー(User)のマルチ認証および、Policy/Gateを用いたアクセス制限の実装。<br><br>
5. N+1問題の対策:<br><br>
・勤務履歴一覧や集計機能において、Eloquentのwith() (Eager Loading)を使用し、データベースへのクエリ負荷を最小限に抑制。<br><br>
